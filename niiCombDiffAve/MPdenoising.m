function [Signal, Sigma] = MPdenoising(data, mask, kernel, sampling, centering)
    ...
    %      10.1002/mrm.26059
    %
    % Modified by C. Baron (2020): slightly reduced aggressiveness of threshold for complex data
    %
    ...
    for nn = 1:numel(x)
        ...
        % C. Baron: I've found that there's a blocky appearance for complex data sometimes, implying some signal components are getting lost
        % Accordingly, I've made this slightly less aggressive.
        if ~isreal(data) && t<R
            t = t+1;
        end
